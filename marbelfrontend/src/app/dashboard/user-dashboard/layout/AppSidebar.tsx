...
const AppSidebar: React.FC = () => {
  ...
  const renderMenuItems = (navItems: NavItem[]) => (
    <ul className="flex flex-col gap-6 text-base lg:text-lg">
      {navItems.map((nav, index) => (
        <li key={nav.name}>
          {nav.subItems ? (
            <button
              onClick={() => handleSubmenuToggle(index)}
              className={`menu-item group py-4 px-4 text-lg ${
  ...
  return (
    ...
    >
          ...
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              ...
              <Image
                ...
                height={40}
              />
              <h1 className="text-xl dark:text-gray-300">Stone Live Stock</h1>
              </div>
            </>
          ) : (
            <Image
      ...
      <div className="flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar flex-1 justify-between">
        <nav className="mb-6">
